...
const Index = () => {
  ...
  // Obtenir le fuseau horaire de la ville depuis les données API
  const cityTimezone = data?.timezone || 'UTC';
  
  // Utiliser Intl.DateTimeFormat pour l'heure locale de la ville
  const formatTimeForCity = (timestamp: string, timezone: string) => {
  ...
  // Obtenir l'heure actuelle dans le fuseau de la ville
  ...
  const currentCityTime = getCurrentCityTime();
  const currentHour = new Date().getHours(); // Pour la logique jour/nuit
  const isDayTime = currentHour >= 6 && currentHour < 20;
  const info = current ? describeWeather(current.weather_code, isDayTime ? 1 : 0) : null;

  // Prévisions horaires CORRECTES avec fuseau horaire de la ville
  const hourlySlice = (() => {
    if (!data) return [];
    
    // Trouver l'heure exacte actuelle dans les données
    const currentCityTime = new Date();
    const currentHourStr = `${currentCityTime.getHours().toString().padStart(2, '0')}:00`;
    const idx = data.hourly.time.findIndex((t: string) => {
      const apiTime = new Date(t);
      const apiHour = apiTime.getHours();
      const currentCityHour = currentCityTime.getHours();
      return apiHour === currentCityHour;
    });
    ...
    for (let i = startIndex; i < endIdx; i++) {
      ...
      // Utiliser Intl.DateTimeFormat avec le fuseau horaire de la ville
      const formattedTime = formatTimeForCity(timestamp, cityTimezone);
      const hourFromFormatted = parseInt(formattedTime.split(':')[0]);
      
      // Utiliser la logique de jour/nuit basée sur l'heure de la ville
      const hourIsDay = hourFromFormatted >= 6 && hourFromFormatted < 20;
      ...
      // Afficher l'heure formatée selon le fuseau de la ville
      const displayTime = i === startIndex ? "mtn" : formattedTime.replace(':', 'H');
